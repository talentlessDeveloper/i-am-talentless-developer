...
import { onLoadVariant, projectVariant } from '../utility/variant';
...
const Project = ({ title, text, img, techLists, reverse, link, live }) => {
  return (
    <motion.div
      ...
      initial="hidden"
      whileInView={'show'}
      variants={onLoadVariant}
      viewport={{ once: true, amount: 0.25 }}
      transition={{ duration: 0.45 }}
    >
      <div
        className={`md:flex ${reverse ? 'flex-row-reverse md:text-right' : ''}`}
      >
        <div className="space-y-8 relative z-[2] md:flex-1">
          <h2 className="font-sans font-bold text-zinc-50 text-xl md:text-3xl  relative  z-[2] overflow-hidden">
            <motion.span className="inline-block" variants={projectVariant}>
              {title}
            </motion.span>
          </h2>
          ...
          >
            {techLists.map((techList) => {
              return (
                <li
                  key={`${title}-${techList}`}
                  className="text-zinc-300 overflow-hidden"
                >
                  <motion.span
        ...
        >
          <img
            ...
            className="grayscale hover:grayscale-0 z-[2] absolute md:relative top-0 left-0 object-cover w-full h-full"
          />
